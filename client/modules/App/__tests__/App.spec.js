...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Header').prop('toggleAddQuestion'), wrapper.instance().toggleAddQuestionSection);
...
test('calling toggleAddQuestionSection dispatches toggleAddQuestion', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddQuestionSection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddQuestion()));
});
